...
const needHooks = ['switch-branch', 'new branch', 'sync branch', 'release']
...
async function actionHookPost ({ git, statusCheck }) {
...
async function entry (params) {
  ...
  try {
    if (fn) {
      needHooks.includes(actionName) && (await actionHookPre(params))
      await fn(params)
      needHooks.includes(actionName) && (await actionHookPost(params))
      ...
      log(style.red(`Action '${actionName}' not found`))
    }
  } catch (err) {
    log(style.red(err))
    log(err)
