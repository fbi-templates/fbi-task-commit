...
const exec = async (
  ...
  if (v) {
    const current = await ctx.utils.exec(
    ...
    console.log(current ? `[${current}] git ${cmd}` : `git ${cmd}`)
  }
...
const s2a = s =>
  (s
    ...
    : null)
// path or all
const validArr = arr => Array.isArray(arr) && arr && arr.length > 0
const pathOrAll = arr => (validArr(arr) ? arr.join(' ') : '.')
