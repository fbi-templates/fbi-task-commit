...
module.exports = {
  ...
  actions: {
    ...
    async 'new branch' ({
      ...
      //  branches['short-lived'][branchType]['baseOn']
      ...
      const localBranches = await status.localBranches()
      if (localBranches.includes(fullName)) {
        throw new Error(
    ...
    async release ({ status, utils, inquirer, exec, statusCheck }) {
      ...
      const base =
        branches['short-lived'][target.split(branches.infix)[0]]['baseOn']
