...
async function hasLocalBranch (name) {
  const ret = await localBranches()
  return name ? ret.includes(name) : ret.length > 0
}
...
async function isBranchUpToDate (branch) {
  const refStr = await exec(`git ls-remote --heads origin ${branch}`)
  const ref = refStr.match(/^(\w+)?/)[1]

  try {
    await exec(`git merge-base --is-ancestor ${ref} HEAD`)
    return true
  } catch (error) {
    if (error.code === 1) {
      return false
...
async function needMerge (b1, b2) {
  return exec(`git rev-list -1 ${b1} --not ${b2}`)
}
